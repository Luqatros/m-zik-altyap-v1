...
client.on('ready', () => {
  ...
  const durumlar = [
    "Sahip=> !Acun#8758",
    "50Krş ver lan trrek"
  ];//burası botun oynuyor kısmıdır her 10 saniye ne oynadığı değişiyir siz bu değiştirebilir veya arttıra bilirsiniz
   setInterval(function () {
    let durum = durumlar[Math.floor(Math.random()*durumlar.length)]
    client.user.setActivity(durum)
  }, 10000)
    client.user.setStatus('dnd');//bu botun durumu göstergesidir dnd rahatsız etmeyindir idle boşta dır 
});
